...
# testy_scaled = pd.DataFrame(scalerX.transform(testX), columns=testX.columns.values).set_index([testX.index.values])
# testy_scaled = pd.DataFrame(scalery.transform(testy), columns=testy.columns.values).set_index([testy.index.values])
...
#This function will need to be done first, to generate the train and test DFs that will be given as 
#parameters for the scaling transformation functions.
...
#The process is: Make the scaler object, and fit it while you're there.
#Apply the scaling object to the train and test set, which is assigned to a new variable.
#Function gives back those scaled dataframes, along with the scaling object fitted to the training dataframe.
...
#This is the one different scaling operation. All the rest do the 1.make scaler, 2.fit scaler, 3.transform process.
#This one is a do-over scaling operation.
